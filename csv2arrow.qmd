...
csvnm = joinpath("data", "311sr.csv.gz")
# size of CSV file in MB
filesize(csvnm) / 10^6
```
...
df = CSV.read(
    ...
    # convert col names to valid symbols
    normalizenames=true, 
    # strings that indicate missing values
    missingstring=[
        "",
        "0 Unspecified",
        "N/A",
    ...
    # use smallest possible Int types
    downcast=true, 
    # strip leading/trailing whitespace
    stripwhitespace=true,
    types=Dict(:Latitude => Float32,
               :Longitude => Float32),
    # Latitude and Longitude are sufficient
    drop=[:Location], 
)
...
```{julia}
first(describe(df, :eltype, :nmissing,
               :nunique, :min), 6)
```
...
```{julia}
#| output: false
first6nm = Arrow.write(
    # returns the name of the file
    joinpath("data", "311sr_first6.arrow"),
    # first index `:` indicating all rows
    df[:, 1:6],   
)
```
...
```{julia}
#| output: false
# like `import pyarrow.feather as fea`
...
```{julia}
arrownm = "311sr.arrow"
Arrow.write(arrownm, df;
            compress=:zstd)
# size of pruned,
# compressed Arrow IPC file in MB
filesize(arrownm) / 10^6
```
...
sort(combine(groupby(df, :Agency),
             nrow => :count), :count; rev=true)
```
...
```{r}
#| eval: false
## pass arrownm from Julia to R
df <- arrow::read_ipc_file($arrownm)
...
```{julia}
#| eval: false
# Julia PyCall equivalent of
# `import polars as pl`
pl = pyimport("polars")

pldf = pl.read_ipc(arrownm,
                   use_pyarrow=true)
