---
title: "From CSV to Arrow: Creating a unified data set for efficient crossplotform analysis"
format:
  jasa-pdf:
    keep-tex: true
    journal:
...
311 service requests, often starts with the ubiquitous CSV file
...
data tables, often provided as CSV (comma-separated value) files.
...
as is the case for the "311 Service Requests in New York City" data
that we will use for illustration, those entering the data seem to be
particularly creative in choosing representations of missing data, 
time-date formats, etc. Experienced data scientists are aware that
...
can be an open-ended task --- it required several trial-and-error
...
processing. Arrow IPC (Inter-Process Communication) is a specific
...
The data were downloaded on July 25, 2023, from 
...
First, attach the packages to be used, assign the name of the CSV
file, and check its size in MB.
...
are like the `factor` type in R or the `categorical` type in 
[Polars](https://pola.rs). The elements of these columns are 
character strings but with considerable repetition of values.
Instead of storing a separate string for each row in these
columns the unique values are stored in a "dictionary" and the
value for each row is determined from an index into the dictionary.
...
```{julia}
arrownm = "311_Service_Requests_2023_03.arrow"
...
Arrow file (less than 18 MB) compared to the original CSV file
(over 156 MB). This reduction could be important for open data
...
and `DCWP` should have been merged in the original data.
...
An alternative, one can use the `dplyr` package in the
...
for metaprogramming and multiple dispatch, which enable more flexible
...
Quarto source of this manuscript:

:   Brief description. (.qmd file)

HTML output:

:   HTML output generated from rending the quarto source (.html file) with full output.

NYC 311 service request data for May, 2023

:   Data set used in the illustration of tro (.csv file).
